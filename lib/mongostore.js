...
function MongoStore(options) {
  ...
  this.mongodb = promisedMongo(options.connectionString);
  this.sessionCollection = this.mongodb.collection(this.options.collection);
...
MongoStore.prototype.get = function(key) {
  ...
  return sessionCollection.findOne({ 'data.token': key }).then(function (session) {
    if (!session) {
      ...
      sessionCollection.remove({ 'data.token': key }).then(function() {
        return {};
      });
    } else {
      return session;
